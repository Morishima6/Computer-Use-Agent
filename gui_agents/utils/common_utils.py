...
_WORKER_SECTION_START = "(Previous action verification)"
...
def call_llm_formatted(generator, format_checkers, **kwargs):
    ...
    while attempt < max_retries:
        ...
        response = normalize_worker_response(response)
...
def normalize_worker_response(response: str) -> str:
    ...
    section_starts = [
        match.start()
        for match in re.finditer(re.escape(_WORKER_SECTION_START), text, re.IGNORECASE)
    ]
    if section_starts:
        text = text[section_starts[-1] :].strip()
