    ...
    def __init__(self, engine_params=None, system_prompt=None, engine=None):
        if engine is None:
            if engine_params is not None:
                ...
                # 创建一个不包含 engine_type 的参数副本，避免传递给引擎构造函数
                filtered_params = {k: v for k, v in engine_params.items() if k != "engine_type"}
                
                if engine_type == "openai":
                    self.engine = LMMEngineOpenAI(**filtered_params)
                elif engine_type == "anthropic":
                    self.engine = LMMEngineAnthropic(**filtered_params)
                elif engine_type == "azure":
                    self.engine = LMMEngineAzureOpenAI(**filtered_params)
                elif engine_type == "vllm":
                    self.engine = LMMEnginevLLM(**filtered_params)
                elif engine_type == "huggingface":
                    self.engine = LMMEngineHuggingFace(**filtered_params)
                elif engine_type == "gemini":
                    self.engine = LMMEngineGemini(**filtered_params)
                elif engine_type == "open_router":
                    self.engine = LMMEngineOpenRouter(**filtered_params)
                elif engine_type == "parasail":
                    self.engine = LMMEngineParasail(**filtered_params)
                else:
