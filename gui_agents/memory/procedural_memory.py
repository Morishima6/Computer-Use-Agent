...
class PROCEDURAL_MEMORY:
    ...
    def construct_simple_worker_procedural_memory(agent_class, skipped_actions):
        ...
        ### CU(part-intent Canonical Unit) Candidate Usage
        - The user message may include sections describing retrieved `CU` candidates from current state similarity and transition history. Use them to identify reusable workflows, likely next subgoals, common paths, and parameter patterns.
    ...
    '''{attr.__doc__}'''
        ...
        (CU Selection)
        If you are using a retrieved CU candidate, explicitly write:
        Selected CU: cu_xxxxx
        Selected Path: path_xx
        Reference Params: {"param_name": "value"}
        If you are not using any CU candidate, write:
        Selected CU: NONE
        Selected Path: NONE
        Reference Params: {}
        ...
        - Your response must contain exactly one occurrence of each required section. Do not output multiple drafts, alternatives, revisions, or corrected versions in the same reply.
        - Do not repeat an earlier answer before or after your final answer.
        - After the single Python code block, end your response immediately.
        When the grounded action uses a descriptive target such as `agent.click(...)`, `agent.scroll(...)`, `agent.type(...)`, or `agent.drag_and_drop(...)`, write the target description so the grounding model can localize it reliably.
        Prefer descriptions that combine multiple anchors from the current screenshot:
            - element type: button, swatch, icon, tab, dropdown item, input box, checkbox, preview box, etc.
            - container or region: dialog name, sidebar, toolbar, panel, section, canvas, table, form, etc.
            - relative position: top-left, top row, third item from the left, below the title, next to the preview box, etc.
            - distinctive visual cues: text label, color, shape, highlight state, selected state, nearby text, nearby controls, etc.
            - expected effect when useful: for example, "which should turn the preview box red" or "which should open the font color palette"        
        - If the task specifies an exact value such as a color, text string, date, or number, preserve that value exactly in your reasoning and grounded action. Do not paraphrase, normalize, or add modifiers unless the user explicitly asked for them.
        ...
        10. Prefer hotkeys and application features over clicking when possible.

        IMPORTANT of CU Reference Mode:
        1. Treat these candidates as planning hints, not as mandatory instructions. If a CU candidate does not fit the current screen or the user's goal, ignore it.
        2. Even when a candidate fits well, you must still output exactly one atomic action for the current turn.
        3. A CU may not include the next immediate action following the completed instruction (it may only partially include it), or it may contain redundant actions; therefore, you should carefully evaluate the contents rather than accepting them wholesale.
    ...
    Your task is to generate a structured reflection that can drive runtime control.

    You must output the following fields exactly:
    Status: (one of) <continue_current_cu, cu_blocked, cu_completed, cu_failed, task_completed>
    Reason: <short explanation>
    Progress Assessment: <short assessment of whether the previous step made progress>
    Intent Predict: <predicted next specific intent for the instruction> 

    Status selection rules:
    - `continue_current_cu`: the current active CU/path still fits the screen and should keep running
    - `cu_blocked`: the current active CU/path is still correct, but the previous grounded action appears to have been executed on the wrong control/target or caused a recoverable wrong UI change; this should trigger a local recovery/rollback instead of retrieval
    - `cu_completed`: the current active CU/path appears finished successfully
    - `cu_failed`: no CU is used (Selected CU: NONE); or the current active CU/path appears mismatched, or no longer applicable, or when recovery is no longer straightforward and a fresh retrieval is preferable
    - `task_completed`: use only when you believe the overall user task is complete

    Suggstion for selection:
    - If the trajectory is repeating with no progress, prefer `cu_blocked`; but this repeating is hopeless, prefer `cu_failed`
    - If the active CU/path still matches the task but the previous action seems to have changed the wrong UI control or wrong target in a recoverable way, prefer `cu_blocked` over `cu_failed`.
    - Do NOT mark `cu_completed` or `task_completed` from the intended action alone, the expected-after-state text alone, a typed value alone, or because a dialog probably closed.

    To be successful, you must follow the rules below:
    - DO NOT suggest any specific future action or UI operation.
    - Your output is for control only, not planning. Keep each field concise and factual
    - Judge completion only from results that are clearly visible in the current screenshot.
    - If a target property change such as color, formatting, text content, or selection state is not clearly visible in the screenshot, do NOT mark completion.
    - Do not assume file modifications or application restarts are errors - they may be legitimate code agent actions
