...
from gui_agents.agents.code_agent import CodeAgent
import logging
...
class OSWorldACI(ACI):
    def __init__(
        ...
        # Add system prompt to guide Claude/GPT models to output coordinates
        grounding_system_prompt = (
            "You are a UI element localization assistant. "
            "Your task is to analyze screenshots and identify precise pixel coordinates of UI elements. "
            "When given a query describing a UI element, respond ONLY with the (x, y) coordinate in the format: (x, y) or x y or x, y. "
            "Do not explain, do not refuse, do not add any other text. Just output the coordinate numbers."
            "When a task requires generating coordinates based on a specified color, coordinates that exactly match that color must be returned; similar or variant colors must not be used (e.g., red ≠ dark red)."
        )
        self.grounding_model = LMMAgent(
        ...
        self.raw_obs: Optional[Dict] = None
        self.model_obs: Optional[Dict] = None

    def _resize_screenshot_for_models(self, screenshot_bytes: bytes) -> bytes:
        ...
        if not grounding_width or not grounding_height:
    ...
    def assign_screenshot(self, obs: Dict):
        self.raw_obs = obs
        self.model_obs = dict(obs)
        screenshot = obs.get("screenshot")
        if screenshot:
            self.model_obs["screenshot"] = self._resize_screenshot_for_models(
                screenshot
            )
        self.obs = self.model_obs

    def get_model_observation(self) -> Dict:
        return self.model_obs if self.model_obs is not None else self.obs
    ...
    def _uses_relative_1000_coordinates(self) -> bool:
        model = str(self.engine_params_for_grounding.get("model") or "").lower()
        return model in {"ui-tars-sft", "ui-tars-7b-sft"} or (
            "ui-tars" in model and "sft" in model
        )
    ...
    def resize_coordinates(self, coordinates: List[int]) -> List[int]:
        ...
        if self._uses_relative_1000_coordinates():
            print('************************************************************Coordinate mode: relative_1000')
            return [
                round(coordinates[0] * self.width / 1000),
                round(coordinates[1] * self.height / 1000),
            ]

        print('************************************************************Coordinate mode: resized_image_pixels')
        return [
    ...
    def call_code_agent(self, task: str = None):
        ...
        **🚨 CRITICAL GUIDELINES:**
        ...
        if self.disable_code_agent:
            raise RuntimeError("call_code_agent is disabled for this run")
